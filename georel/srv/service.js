...
const _callS4H = async function(bupaId) {
    return new Promise((resolve, reject) => {
        BusinessPartner.requestBuilder()
            ...
            .execute({ destinationName: '<destination-instance>' }) // the name of destination instance
            .then((bupa) => {
                if (bupa) {
                ...
                console.error('   [S4H]: called S4H API (via XF) Tried to find BuPa in S4 system, but request to S4H system failed: ' + err.stack)
            })
    })
}
